...
# ------------------------
if len(sys.argv)<2:
    print("用法: python analyze_tcp_dpkt_engineer.py target.pcap")
    sys.exit(1)
...
for i,(k,v) in enumerate(flow_results.items()):
    times, wins = zip(*v['win_ts']) if v['win_ts'] else ([],[])
    if times and wins:
        ax1.plot(times, wins, color=colors[i%20], alpha=0.5, label=f"Flow {k[0]}->{k[2]}")
    if v['timestamps']:
        rtts = [j-i for i,j in zip(v['timestamps'][:-1],v['timestamps'][1:])]
        ax2.plot(v['timestamps'][1:], rtts, color=colors[i%20], linestyle='--', alpha=0.3)
ax1.set_xlabel("Time (s)")
ax1.set_ylabel("Window size", color='blue')
ax2.set_ylabel("Approx RTT", color='red')
ax1.set_title("TCP Flow Window & RTT Trend")
...
plt.tight_layout()
plt.savefig("tcp_comprehensive_analysis.png")
plt.show()

# ------------------------
# 输出 CSV
# ------------------------
with open('tcp_flow_analysis_engineer.csv','w',newline='') as f:
    writer = csv.writer(f)
...
print("分析完成，已生成 tcp_flow_analysis_engineer.csv 和综合图 tcp_comprehensive_analysis.png")
